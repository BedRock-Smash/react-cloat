...
export const useProducts = () => {
    ...
    const sortQuery = activeSort === "alphabetical" ? "_sort=title" : "_sort=price";
    const categoryQuery = activeCategory === "all" ? "" : `type=${activeCategory} `
    const { data: products, error, isLoading } = useSWR(`${ApiRoutes.PRODUCTS}?${sortQuery}&${categoryQuery}`, fetcher)
